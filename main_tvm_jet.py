...
def get_alpr(region):
    # load alpr model
    alpr = Alpr(region, "/etc/openalpr/openalpr.conf","/home/nvidia/lpr/openalpr/runtime_data")
...
def build():
    graph = open("jetson-model/model_opt.json").read()
    lib = tvm.runtime.load_module("jetson-model/model_opt.so")
    params = bytearray(open("jetson-model/model_opt.params", "rb").read())
